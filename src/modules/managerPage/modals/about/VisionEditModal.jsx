...
const VisionEditModal = ({ vision, reloadData }) => {
		...
		try {
			...
			if (!file) {
				...
				await updateDoc(docRef, {
					"about.vision.content": String(value),
				});
			} else if (file) {
				const stamp = timestamp();
				// delete old file
				const deleteRef = ref(storage, `images/${vision?.image?.name}`);
	...
	return (
		<div className="flex flex-col h-full">
			<SectionTitle>理念の編集</SectionTitle>
			<div className="flex flex-col flex-1 gap-4 overflow-y-auto">
				...
				<div>
					<ModalFieldTitle>理念</ModalFieldTitle>
					<QuillEditor
						className=""
						theme="snow"
						value={value}
