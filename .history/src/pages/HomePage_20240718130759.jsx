...
const HomePage = () => {
  ...
  useEffect(() => {
    async function fetchImg() {
      try {
        const slideRes = await axios.get('https://cors-anywhere.herokuapp.com/https://script.google.com/macros/s/AKfycbzOSIFIbV-Ew0bpoGnmsZoQhl4txvxSkZc6ekMywDUvIJ5929eM7GbCXnTZVdtenDXx/exec?action=getSlide');
        console.log(slideRes.data);  // Log the response to check the structure
        const fetchImgData = { slideImg: slideRes.data.data };
        setApiData(fetchImgData);
      } catch (err) {
  ...
  useEffect(() => {
    if (data && data.tours && data.tours.length > 0) {
      ...
      console.log(data.tours[0]);  // Log the tour data
    }
  }, [data]);
...
// }, [data]) 
  ...
  return (
    <div>
      ...
      {tourImg.images && tourImg.images.length > 0 && (
      <CarouselImages
