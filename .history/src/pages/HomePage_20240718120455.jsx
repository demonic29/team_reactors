...
const HomePage = () => {
  ...
  const [tourImg, setTourImgs] = useState({})
  
  useEffect(() => {
    async function fetchImg () {
      try{
        const slideRes = await Promise.all(axios.get(`${API.GET_DATA}?action=getSlide`))
    ...
    if(apiData && apiData.tours && apiData.tours.length){
      setTourImg(apiData.tous[0])
    }
  }, [apiData])
